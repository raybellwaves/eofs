...
class Eof(object):
    ...
    def eofsAsCovariance(self, neofs=None, pcscaling=1):
        ...
        **Examples:**
        ...
        """
        eofs = self._solver.eofsAsCovariance(neofs, pcscaling) # numpy array
        # variance explained
        varexpl = self._solver.varianceFraction(neofs) * 100
        # Create xarray
        eofdim = xr.IndexVariable('mode', range(eofs.shape[0]),
                               attrs={'long_name': 'eof_mode_number'})
        coords = [eofdim] + self._coords
        ...
        eofs.coords.update({coord.name: (coord.dims, coord)
                            for coord in self._space_ndcoords})
        # Add variance explain as an attribute
        eofs.attrs['var_expl'] = varexpl
        return eofs
